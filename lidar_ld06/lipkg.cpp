...
void LiPkg::ToLaserData(std::vector<PointData> src)
{
  ...
  angle_increment = ANGLE_TO_RADIAN(mSpeed/4500);
  /*Calculate the number of scanning points*/
  beam_size = ceil((angle_max - angle_min) / angle_increment);

  /*First fill all the data with Nan*/
  ranges.assign(beam_size, std::numeric_limits<float>::quiet_NaN());
  //output.intensities.assign(beam_size, std::numeric_limits<float>::quiet_NaN());

  ranges.resize(beam_size);
  //std::vecotr<float> ranges(beam_size, 0.0);
  ...
  for (auto point : src)
  {
    ...
    if (index >= 0 && index < beam_size)
    {
      ...
      //反射强度暂时不用
      //intensities[index] = point.confidence;
    }
  }
}
